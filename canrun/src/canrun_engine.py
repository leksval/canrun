...
class CanRunEngine:
    ...
    async def check_game_compatibility(self, game_name: str, use_cache: bool = True) -> CanRunResult:
        ...
        # Get display resolution for accurate performance prediction
        display_resolution = hardware_specs.display_resolution if hasattr(hardware_specs, 'display_resolution') else None
        if not display_resolution:
            # Fallback to DisplayDetector for resolution detection
            try:
                ...
                display_resolution = {'width': 1920, 'height': 1080}  # Default fallback
        
        # Capture resolution classification before passing to performance predictor
        width = display_resolution.get('width', 1920)
        height = display_resolution.get('height', 1080)
        
        # Classify resolution using DisplayDetector
        from canrun.src.display_detector import DisplayDetector
        detector = DisplayDetector()
        ...
        game_requirements_dict = {
            "game_name": game_requirements.game_name,  # CRITICAL: Add game_name for ML model
