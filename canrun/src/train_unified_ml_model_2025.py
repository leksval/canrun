...
class UnifiedMLTrainer2025:
    ...
    def create_stacking_ensemble(self) -> List[Tuple[str, Any]]:
        ...
        # XGBoost with improved device consistency
        if XGBOOST_AVAILABLE:
            try:
                # Try GPU first with proper device consistency
                test_model = xgb.XGBRegressor(device='cuda', tree_method='hist', random_state=42)
                test_model.fit([[1, 2], [3, 4]], [1, 2])  # Quick test
                
                # Set device on booster to prevent prediction warnings
                if hasattr(test_model, 'get_booster'):
                    booster = test_model.get_booster()
                    booster.set_param('device', 'cuda')
                
                logger.info("   XGBoost GPU enabled (with device consistency)")
                xgb_model = xgb.XGBRegressor(
                    n_estimators=500, max_depth=8, learning_rate=0.1, random_state=42,
                    device='cuda', tree_method='hist',
                    subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=1.0,
                    enable_categorical=False  # Disable categorical to avoid device issues
                )
                base_models.append(('xgb_gpu', xgb_model))
    ...
    def _analyze_feature_importance(self, model, feature_names: List[str]):
        ...
        try:
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                used_model_name = "stacking ensemble"
            else:
                # For stacking models, get importance from the first base estimator that has it
                # Skip meta-learner as it only has coefficients for base model outputs, not original features
                feature_importance_found = False
                
                # Handle different stacking ensemble structures
                estimators_list = []
                if hasattr(model, 'estimators_') and model.estimators_:
                    estimators_list = model.estimators_
                elif hasattr(model, 'named_estimators_') and model.named_estimators_:
                    estimators_list = list(model.named_estimators_.items())
                ...
                for item in estimators_list:
                    ...
                    if isinstance(item, tuple) and len(item) == 2:
                        ...
                        # Skip if not a proper name-estimator pair
                        continue
                    ...
                    if hasattr(estimator, 'feature_importances_'):
                        ...
                        logger.info(f"  Using feature importance from {name} (base model)")
                        break
                
                if not feature_importance_found:
                    logger.info("  No tree-based base estimators with feature_importances_ found")
                    logger.info("  Meta-learner coefficients represent base model weights, not original feature importance")
                    
                    # Show base model weights from meta-learner instead
                    if hasattr(model.final_estimator_, 'coef_'):
                        meta_coefs = np.abs(model.final_estimator_.coef_)
                        base_model_names = [name for name, _ in model.estimators_]
                        
                        logger.info("  Base Model Weights from Meta-Learner:")
                        for i, (name, weight) in enumerate(zip(base_model_names, meta_coefs)):
                            logger.info(f"    {i+1:2d}. {name}: {weight:.4f}")
            ...
            if len(feature_names) != len(importances):
                ...
                logger.warning(f"  This suggests an issue with feature engineering or model structure")
                
                # Skip analysis if significant mismatch
                if abs(len(feature_names) - len(importances)) > 5:
                    logger.error(f"  Large feature count mismatch - skipping feature importance analysis")
                    return
                
                # For small mismatches, truncate to shorter length
                min_length = min(len(feature_names), len(importances))
                feature_names = feature_names[:min_length]
                importances = importances[:min_length]
                logger.info(f"  Truncated to {min_length} features for analysis")
            ...
            logger.info(f"  Top 15 Most Important Features (from {used_model_name}):")
    ...
    def _ensure_xgb_device_consistency(self, model):
        """Ensure XGBoost models have consistent device settings for predictions"""
        try:
            if hasattr(model, 'estimators_'):
                # Handle stacking ensemble
                for name, estimator in model.estimators_:
                    if 'xgb' in name.lower() and hasattr(estimator, 'get_booster'):
                        booster = estimator.get_booster()
                        # Get current device from model
                        current_device = getattr(estimator, 'device', 'cpu')
                        booster.set_param('device', current_device)
                        logger.debug(f"  Set {name} booster device to {current_device}")
            elif hasattr(model, 'get_booster'):
                # Handle single XGBoost model
                booster = model.get_booster()
                current_device = getattr(model, 'device', 'cpu')
                booster.set_param('device', current_device)
                logger.debug(f"  Set XGBoost booster device to {current_device}")
        except Exception as e:
            logger.debug(f"  XGBoost device consistency check failed: {e}")
    ...
    def _generate_comprehensive_predictions(self, model, calibrator, domain_correction,
        ...
        # Ensure XGBoost device consistency before predictions
        self._ensure_xgb_device_consistency(model)
